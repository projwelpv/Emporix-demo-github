...
class ShopingCart extends Component {
  ...
  PlusQty = (Index,pid) => {
    ...
    let pricelist = [];
      this.props.allPrices.map((each, index) => {
        if (each.itemId.id === pid) {
          pricelist.push(each);
        }
      });

      if (pricelist.length > 0) {
        this.props.updateCartQty(this.props.cartID, pid, pricelist,  parseInt(UpdatedCart[Index].Qty));
      }
  ...
  MinusQty = (Index,pid) => {
    ...
    if (UpdatedCart[Index].Qty != 1) {
      ...
      deleteCartItemsNew(pid,this.props.allPrices,this.props.cartID)
    }
  };
                              ...
                              <div className="quantity">
                                ...
                                <div className="quantity-nav">
                                  <Link
                                    ...
                                    onClick={() => this.PlusQty(index,CartItem.ProductID)}
                                  >
                                    +
                                  </Link>
                                  <Link
                                    className="quantity-button quantity-down"
                                    onClick={() => this.MinusQty(index,CartItem.ProductID)}
                                  >
                                    -
                                  </Link>
                                </div>
